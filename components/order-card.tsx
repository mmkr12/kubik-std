...
export function OrderCard({ order }: { order: Order }) {
  return (
    <Link href={`/production/${order.id}`} className="group block">
      <div className="overflow-hidden rounded-2xl bg-navy-900 card-shadow card-shadow-hover transition-transform duration-300 group-hover:-translate-y-1">
        ...
        <div className="space-y-2 p-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-semibold text-white">{order.company_name}</h3>
            <Badge>В работе</Badge>
          </div>
          {order.install_date && (
            <p className="text-sm text-white/50">Монтаж {formatDate(order.install_date)}</p>
          )}
        </div>
      </div>
    </Link>
